...
interface UseTableConfig<T = any> {
  ...
  updateStatusApi?: (status: any, ids: any) => Promise<T>
  importExcelApi?: (option: any) => Promise<T>
  // 返回数据格式配置
...
export const useTable = <T = any>(config?: UseTableConfig<T>) => {
  ...
  const updateStatus = async (enable: number, ids: string | number | string[] | number[]) => {
    await (config?.updateStatusApi && config?.updateStatusApi(enable, ids))
  ...
  const methods = {
    getList: async (param: any = {}) => {
      ...
      const res = await config?.getListApi(params).finally(() => {
        tableObject.loading = false
      })
      if (res) {
        const list = (res as unknown as ResponseType).list
        if (config?.handleRow) {
          list.forEach((it) => config?.handleRow(it))
        }
    ...
    // 批量操作
    handleTableAction: async (fun: (ids: number[])=>void) => {
      const tableRef = await getTable()
      if (!tableRef?.selections.length) {
        ElMessage.warning(t('common.delNoData'))
        return
      }
      const idList = tableRef?.selections.map((item) => item.id);
      fun(idList);
    },
    // 批量删除数据
    delList: async (
      ...
      if (multiple) {
        ...
        idList = tableRef?.selections.map((item) => item.id)
      }
      if (message) {
    ...
    // 更新状态
    updateStatus: async (enable: 0 | 1, ids: string | number | string[] | number[] | null) => {
      ElMessageBox.confirm(`是否${enable ? '启用' : '禁用'}该条数据`, '系统提示', {
        confirmButtonText: t('common.ok'),
        cancelButtonText: t('common.cancel'),
        type: 'warning'
      }).then(async () => {
        await updateStatus(enable, ids!)
      })
    },
    // 导出列表
    ...
    // 导入excel
    importExecl: async (event: any) => {
      const files = event.target.files
      if (files.length > 0) {
        const file = files[0]
        const formData = new FormData()
        formData.append('file', file)
        if (config?.importExcelApi) {
          const res: any = await config?.importExcelApi(formData)
          const { data } = res
          const { successCount, failCount, failNames } = data
          ElMessageBox.alert(
            `${successCount}条数据导入成功, ${failCount}条数据导入失败, 失败行: ${failNames.join(',')}`,
            '上传结果'
          )
          methods.getList()
        }
