      ...
      if (searchSchemaItem.api) {
        searchRequestTask.push(async () => {
          ...
          if (res) {
            ...
            if (index !== -1) {
              const labelAlias = searchSchemaItem?.componentProps?.optionsAlias?.labelField
      ...
      if (formSchemaItem.api) {
        formRequestTask.push(async () => {
          ...
          if (res) {
            ...
            const labelAlias = formSchemaItem?.componentProps?.optionsAlias?.labelField
            const labelAliasStr = labelAlias && Array.isArray(labelAlias) ? labelAlias[0] : labelAlias
            if (index !== -1) {
              allSchemas.formSchema[index]!.componentProps!.options = filterOptions(res, labelAliasStr)
            }
          }
        })
      }
