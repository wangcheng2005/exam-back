...
import { MergeCellsType, Pagination, TableColumn, TableSetPropsType, TableSlotDefault } from '@/types/table'
...
export default defineComponent({
  ...
  props: {
    ...
    // 是否合并单元格
    mergeCells: {
      type: Object as PropType<MergeCellsType>,
      default: () => {}
    }
  ...
  setup(props, { attrs, slots, emit, expose }) {
    ...
    const objectSpanMethod = ({ rowIndex, columnIndex }: any) => {
      const tableList = unref(getProps).data
      const mergeCells = unref(getProps).mergeCells
      if(!mergeCells){
        return {
          rowspan: 1,
          colspan: 1
      ...
      const { col=[], fieldName=[] } = mergeCells;
      if(!col || !fieldName || tableList.length === 0){
        return {
          rowspan: 1,
          colspan: 1
        }
      ...
      if (col.includes(columnIndex)) {
        ...
        let rIndex = 0
        let count = 0
        const rowItem = tableList[0];
        const cIndex = col.indexOf(columnIndex);
        const fieldname = fieldName[cIndex];
        let tName = rowItem[fieldname]
        tableList.forEach((item, index) => {
          if (item[fieldname] === tName) {
            count++
          } else {
            list.push({ index: rIndex, count: count })
            rIndex = index
            count = 1
        ...
        for (let i = 0; i < list.length; i++) {
          if (rowIndex === list[i].index) {
            return {
              rowspan: list[i].count,
              colspan: 1
            }
          } else if (rowIndex > list[i].index && rowIndex < list[i].index + list[i].count) {
            return {
              rowspan: 0,
              colspan: 0
            }
