import { DictOptionType } from '@/store/modules/dict'
...
/**
 * 根据DictOptionType[]获取对应的值
 */
export const getDictOptionValue = (dictOptions: DictOptionType[], value: any): string => {
  for (const dict of dictOptions) {
    if (dict.value === value) {
      return dict.label
    } else if (dict.children) {
      const item = getDictOptionValue(dict.children, value)
      if (item) {
        return item
      }
    }
...
export const getImageUrl = (url: string) => {
  ...
  return url ? import.meta.env.VITE_FILE_BASEPATH + url : '';
}